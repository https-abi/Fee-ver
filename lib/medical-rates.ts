...
export async function searchMedicalRates(
  ...
  try {
    ...
    try {
      // PostgreSQL fuzzy search using similarity and trigram matching
      // This requires pg_trgm extension to be enabled in your database
...
export async function getBenchmarkAnalysis(
  ...
  // Check if overpriced (charged amount is above max_rates or significantly above rates)
  const isOverpriced =
    chargedAmount > maxPrice ||
    (chargedAmount > benchmarkPrice && variance > 20);

  console.log(`   📊 Benchmark Analysis Results:`);
  console.log(`      - Benchmark price: ₱${benchmarkPrice.toLocaleString()}`);
  console.log(`      - Min price: ₱${minPrice.toLocaleString()}`);
  console.log(`      - Max price: ₱${maxPrice.toLocaleString()}`);
  console.log(`      - Variance: ${varianceText}`);
  console.log(`      - Is overpriced: ${isOverpriced ? "YES ⚠️" : "NO ✓"}`);
  console.log(
...
export async function initializeDatabase() {
  ...
  try {
    ...
    try {
      ...
      // Create GIN index on description for better fuzzy search performance
